...
public class BookingSystem
{
  ...
  public void makeReservation(int covers, Date date, Time time, int tno,
			      String name, String phone,String menu)
  {
    if (!doubleBooked(time, tno, null) && !overflow(tno, covers)) {
      Booking b
	    = restaurant.makeReservation(covers, date, time, tno, name, phone, menu) ;
      currentBookings.addElement(b) ;
      notifyObservers() ;
    }
  }
 
  public void makeWalkIn(int covers, Date date, Time time, int tno,String menu)
  {
    if (!doubleBooked(time, tno, null) && !overflow(tno, covers)) {
      Booking b = restaurant.makeWalkIn(covers, date, time, tno,menu) ;
      currentBookings.addElement(b) ;
      notifyObservers() ;
    }
  }
