...
public class BookingMapper
{
  ...
  public Vector getBookings(Date date)
  {
	...
	PersistentReservation r
	  = new PersistentReservation(oid, covers, bdate, btime, t, c, atime, menu) ;
	v.add(r) ;
      }
      rset.close() ;
...
Time btime = rset.getTime(4) ;
int table = rset.getInt(5) ;
	String menu=rset.getString(6);
	PersistentTable t = TableMapper.getInstance().getTableForOid(table) ;
	PersistentWalkIn w
	  = new PersistentWalkIn(oid, covers, bdate, btime, t, menu) ;
	v.add(w) ;
      }
      rset.close() ;
     stmt.close() ;
